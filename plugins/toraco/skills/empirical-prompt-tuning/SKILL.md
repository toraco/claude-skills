...
description: agent 向けテキスト指示（skill / slash command / タスクプロンプト / CLAUDE.md 節）を、バイアスを排した実行者に動かしてもらい、両面評価で反復改善する。skill やプロンプトを新規作成・大幅改訂した直後、または agent の挙動が期待通りにならないときに使う。
---
...
- `superpowers:writing-skills`（外部プラグイン、現在未インストール）— skill 作成時の TDD アプローチ。本 skill の「subagent で baseline → 修正 → 再実行」と本質的に同じ
- `retrospective-codify`（現在未作成）— タスク後の学び固定化。本 skill はプロンプト開発中、retrospective-codify はタスク終了後、と使い分ける
- `superpowers:dispatching-parallel-agents`（外部プラグイン、現在未インストール）— 複数シナリオを並列で走らせるときの作法
